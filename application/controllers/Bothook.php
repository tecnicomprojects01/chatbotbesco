...
class Bothook extends CI_Controller {
	...
	function chatbot($key){
		...
		$mensajebienvenida = applib::get_all('mensaje',applib::$mensajebienvenida_table,array('id_msj' => 1));

		foreach ($mensajebienvenida as $msjbn) {
			$this->fbbot->sendMessage($cliente['fb_id'],'texto',array('texto' => $msjbn['mensaje']));
		}
	...
	private function send_comunicar_asesor($sender,$key){
        ...
        $botones[] = [
           	"type"					=> "web_url",
			"url" 					=> 'https://chat.tecnicom.pe/index.php/sender/contacto/'.$proyecto['seo'].'/L5437788/'.$cliente['id'],
        ...
        if($proyecto['aceptar_llamadas'] == 1 && ($this->get_hora() >= "10" && $this->get_hora() <= "17")){
        	...
        	$botones[] = [
           		"type"					=> "web_url",
				"url" 					=> 'https://chat.tecnicom.pe/index.php/sender/llamada/'.$proyecto['seo'].'/L5437788/'.$cliente['id'],
